<template>
  ...
  <div class="px-6 py-3 flex justify-between items-center">
    ...
    <div class="flex gap-7 items-center">
        ...
        <form>
          ...
          <div class="relative">
            <input
              type="search"
              id="default-search"
              class="block w-full p-2 text-sm text-gray-900 border-none drop-shadow-md rounded-lg bg-gray-50 dark:bg-gray-700 dark:placeholder-gray-light dark:text-white dark:focus:ring-blue-main dark:focus:border-blue-main"
              placeholder="Buscar"
              required
            />
            <button
              type="submit"
              class="text-gray absolute right-2.5 bottom-2.5 focus:ring-4 font-medium rounded-lg text-sm"
            >
              <svg
                aria-hidden="true"
      ...
      <div class="flex flex-col justify-center items-center text-gray">
        <span class="font-semibold">Contactanos</span>
      ...
      <div class="flex">
        <a
          ...
          :key="index"
          :href="social.href"
          ><img :src="social.img" :alt="social.alt" class="h-8" />
        </a>
      </div>
    </div>
  </div>
  ...
  <div class="flex justify-between py-2 px-6">
    <div class="flex gap-6">
      <a v-for="(category, index) of categories" href="" class="font-semibold text-gray hover:text-blue-main transition ease-in-out">
        {{ category.name }}</a
      >
    </div>
    <div class="flex gap-4">
      <a href="">
        <svg
          class="h-6 fill-gray hover:fill-blue-main transition ease-in-out" 
          viewBox="0 0 20 20"
          version="1.1"
          xmlns="http://www.w3.org/2000/svg"
